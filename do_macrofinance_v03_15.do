...
gen pasivos_cp = (dep_vp + dep_ch) / activo
label variable pasivos_cp "Proxi de pasivos de corto plazo"

*** Cálculo del Income Gap basado en Gómez et al. (2021) ***
gen income_gap = (activos_corto_plazo * activo - pasivos_corto_plazo * activo) / activo
...
graph export "output/figures/activos_cp_subs_mean.pdf", replace
...
graph bar (mean) pasivos_cp, ///
    ...
    blabel(bar, format(%9.2f)) ///
    note("Fuente: ASFI") ///
    note("Nota: Pasivos de Corto Plazo = (Depósitos a la Vista + Cajas de Ahorro) / Activos Totales")

graph save "output/figures/pasivos_cp_subs_mean.gph", replace
graph export "output/figures/pasivos_cp_subs_mean.pdf", replace
...
graph export "output/figures/income_gap_subs_mean.pdf", replace
